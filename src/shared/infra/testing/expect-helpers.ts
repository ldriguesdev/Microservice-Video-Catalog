...
expect.extend({
  containsErrorMessage(expected: Expected, received: FieldsErrors) {
    if (typeof expected === "function") {
      try {
        expected();
        return { pass: false, message: () => "Expected validation error but none was thrown" };
      } catch (e) {
      ...
      if (validated) {
        return { pass: false, message: () => "Expected validation error but validator returned valid" };
      }
